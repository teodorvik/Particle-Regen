...
%             end
            ...
            if (firstBall.position(1) + firstBall.radius + secondBall.radius > secondBall.position(1) && firstBall.position(1) < secondBall.position(1) + firstBall.radius + secondBall.radius && firstBall.position(2) + firstBall.radius + secondBall.radius > secondBall.position(2) && firstBall.position(2) < secondBall.position(2) + firstBall.radius + secondBall.radius)
                ...
                if (distance < firstBall.radius + secondBall.radius)
                    ...
                    collisionPointX = ((firstBall.position(1) * secondBall.radius) + (secondBall.position(1) * firstBall.radius)) / (firstBall.radius + secondBall.radius);
                    collisionPointY = ((firstBall.position(2) * secondBall.radius) + (secondBall.position(2) * firstBall.radius)) / (firstBall.radius + secondBall.radius);
                    
                    newVelX1 = (firstBall.velocity(1) * (firstBall.mass - secondBall.mass) + (2 * secondBall.mass * secondBall.velocity(1))) / (firstBall.mass + secondBall.mass);
                    newVelY1 = (firstBall.velocity(2) * (firstBall.mass - secondBall.mass) + (2 * secondBall.mass * secondBall.velocity(2))) / (firstBall.mass + secondBall.mass);
                    newVelX2 = (secondBall.velocity(1) * (secondBall.mass - firstBall.mass) + (2 * firstBall.mass * firstBall.velocity(1))) / (firstBall.mass + secondBall.mass);
                    newVelY2 = (secondBall.velocity(2) * (secondBall.mass - firstBall.mass) + (2 * firstBall.mass * firstBall.velocity(2))) / (firstBall.mass + secondBall.mass);
                    
                    particleSystem(i).position(1) = firstBall.position(1) + newVelX1;
                    particleSystem(i).position(2) = firstBall.position(2) + newVelY1;
                    ball.position(1) = secondBall.position(1) + newVelX2;
                    ball.position(2) = secondBall.position(2) + newVelY2;
                end
            end
        end
	end
